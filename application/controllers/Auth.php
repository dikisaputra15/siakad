...
class Auth extends CI_Controller
{
	...
	public function __construct()
	{
		...
		}elseif (!empty($this->session->userdata('email')) && $this->session->userdata('role_id') == 10) {
			$this->Flasher_model->flashdata('You login with <a href="' . base_url('user') . '">User</a> Or <a href="' . base_url('auth/logout') . '">Logout</a>', 'Warning', 'warning');
		}
	}
	...
	public function forgot_password()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($user){
				...
				$this->Flasher_model->flashdata('password anda sekarang adalah admin silahkan login dan ganti password anda', 'Success', 'success');
				redirect('auth');
			}else{
				$this->Flasher_model->flashdata('Email tidak sesuai', 'Failed', 'danger');
	...
	private function _sendEmail($token, $type)
	{
		...
		$confing = [
			...
			'smtp_user' => 'dikisaputra396@gmail.com',
			'smtp_pass' => 'diki15071996@',
			'smtp_port' => 465,
			'mailtype'  => 'html',
			'charset'   => 'utf-8',
			'newline'   => "\r\n"
		...
		$this->email->from('dikisaputra396@gmail.com', 'Diki Saputra');
		$this->email->to($email);
