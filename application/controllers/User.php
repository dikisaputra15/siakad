...
class User extends CI_Controller
{
	...
	public function delete($id = -1)
	{
		...
		if (is_null($this->db->get_where('user', ['id' => $id])->row_array())) {
			...
			$this->db->where('id', $id);
			$this->db->delete('user');
			$this->Flasher_model->flashdata('user deleted','Succes','warning');
			redirect('menu/usermanagement');
		}
	}

	public function user_akses_non($id)
	{
		$this->db->set('is_active', 0);
		$this->db->where('id', $id);
		$this->db->update('user');
		$this->Flasher_model->flashdata('Your account is Non Active','Succes','warning');
		redirect('menu/usermanagement');
	}

	public function user_akses_aktif($id)
	{
		$this->db->set('is_active', 1);
		$this->db->where('id', $id);
		$this->db->update('user');
		$this->Flasher_model->flashdata('Your account is active', 'Success', 'success');
		redirect('menu/usermanagement');
	}
